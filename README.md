...
This project uses the ["Progressive Web App"](https://web.dev/explore/progressive-web-apps) technology built into most device browsers to quickly generate a working PWA hostable from a user's own device created from one page of HTML. The users' created app and its associated data are stored directly on their device using the browser's local storage, are not available to the public, and not stored on a web server.
...
# Why
Sometimes I have a simple app idea that I want to generate as a single page of HTML and this lets me quickly have it with me. With LLMs its much easier to take an app from an idea to actual code but hosting it somewhere just to use a couple times is a lot of trouble. Imagine creating a custom app just to help with a game at a birthday party, or a quick browser game just for fun.
